<!-- footer section starts -->
<section class="footer">
    <div class="box-container">
        <div class="box">
            <a href="tel:1234567890"><i class="fas fa-phone"></i> +123-456-7890</a>
            <a href="tel:111223333"><i class="fas fa-phone"></i> +111-222-3333</a>
            <a href="mailto:nelo.code155@gmail.com"><i class="fas fa-envelope"></i> nelo.code155@gmail.com</a>
            <a href="#"><i class="fas fa-map-marker-alt"></i> Negros Occidental, Philippines - 6100</a>
        </div>

        <div class="box">
            <a href="#home">Home</a>
            <a href="#about">About</a>
            <a href="#reservation">Reservation</a>
            <a href="#gallery">Gallery</a>
            <a href="#contact">Contacts</a>
            <a href="#reviews">Reviews</a>
        </div>

        <div class="box">
            <a href="#">facebook <i class="fab fa-facebook-f"></i></a>
            <a href="#">twitter <i class="fab fa-twitter"></i></a>
            <a href="#">linkedin <i class="fab fa-linkedin"></i></a>
            <a href="#">youtube <i class="fab fa-youtube"></i></a>
        </div>
    </div>

    <div class="credit">
        &copy; copyright @ 2025 by Nelo Code | all rights reserved
    </div>
    
    <!-- Development credit - optional -->
    <div class="dev-credit" style="text-align: center; padding-top: 1rem; font-size: 1.2rem; color: #888;">
        <small>Developed with assistance from DeepSeek & Claude AI</small>
    </div>
</section>
<!-- footer section ends -->